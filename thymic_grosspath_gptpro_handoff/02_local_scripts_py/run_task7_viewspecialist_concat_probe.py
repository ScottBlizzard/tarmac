...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Task7 cut/outer specialist routed probe using frozen DINO features.")
    ...
    parser.add_argument("--cut-global-weight", type=float, default=0.1)
    parser.add_argument("--outer-global-weight", type=float, default=0.1)
    parser.add_argument("--mixed-cut-weight", type=float, default=0.45)
    parser.add_argument("--mixed-outer-weight", type=float, default=0.45)
    parser.add_argument("--mixed-global-weight", type=float, default=0.1)
    return parser.parse_args()


def fit_probe(features: np.ndarray, labels: np.ndarray) -> Pipeline:
    clf = Pipeline(
        steps=[
...
def safe_predict(model: Pipeline | None, features: np.ndarray) -> np.ndarray | None:
    if model is None:
        return None
    return model.predict_proba(features)


def blend_probs(parts: list[tuple[np.ndarray | None, float]]) -> np.ndarray:
    valid = [(arr, weight) for arr, weight in parts if arr is not None and weight > 0]
    if not valid:
        raise ValueError("No valid probability arrays to blend.")
...
def compute_metrics(df: pd.DataFrame) -> dict[str, float]:
    ...
    prob_high = df["prob_high_risk_group"].astype(float).to_numpy()
...
def main() -> None:
    ...
    for fold_id in fold_ids:
        ...
        global_clf = fit_probe(x_train, y_train)

        # Specialist training sets
        cut_train_mask = np.isin(train_view, ["cut_surface", "mixed"])
        outer_train_mask = np.isin(train_view, ["outer_surface", "mixed"])

        cut_clf = None
        outer_clf = None
        if cut_train_mask.sum() >= 16 and len(np.unique(y_train[cut_train_mask])) >= 2:
            cut_clf = fit_probe(x_train[cut_train_mask], y_train[cut_train_mask])
        if outer_train_mask.sum() >= 16 and len(np.unique(y_train[outer_train_mask])) >= 2:
            outer_clf = fit_probe(x_train[outer_train_mask], y_train[outer_train_mask])

        global_val = global_clf.predict_proba(x_val)
        global_test = global_clf.predict_proba(x_test)
        cut_val = safe_predict(cut_clf, x_val)
        cut_test = safe_predict(cut_clf, x_test)
        outer_val = safe_predict(outer_clf, x_val)
        outer_test = safe_predict(outer_clf, x_test)

        def route_one(view_name: str, g: np.ndarray, c: np.ndarray | None, o: np.ndarray | None) -> np.ndarray:
            if view_name == "cut_surface":
                return blend_probs([(c, 1.0 - args.cut_global_weight), (g, args.cut_global_weight)])
            if view_name == "outer_surface":
                return blend_probs([(o, 1.0 - args.outer_global_weight), (g, args.outer_global_weight)])
            if view_name == "mixed":
                return blend_probs(
                    [
                        (c, args.mixed_cut_weight),
                        (o, args.mixed_outer_weight),
                        (g, args.mixed_global_weight),
                    ]
                )
            return g

        def route_batch(view_names: np.ndarray, g: np.ndarray, c: np.ndarray | None, o: np.ndarray | None) -> np.ndarray:
            out = []
            for idx, view_name in enumerate(view_names):
                out.append(route_one(view_name, g[idx : idx + 1], None if c is None else c[idx : idx + 1], None if o is None else o[idx : idx + 1]))
            return np.concatenate(out, axis=0)

        val_probs = route_batch(val_view, global_val, cut_val, outer_val)
        test_probs = route_batch(test_view, global_test, cut_test, outer_test)
        ...
        write_json(
            fold_dir / "fold_summary.json",
            {
                ...
                "cut_specialist_train": int(cut_train_mask.sum()),
                "outer_specialist_train": int(outer_train_mask.sum()),
                "cut_clf_fitted": cut_clf is not None,
                "outer_clf_fitted": outer_clf is not None,
    ...
    metrics = compute_metrics(oof_case)
