...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Task7 directional flip/no-flip policy for routed cases.")
    parser.add_argument(
        "--case-scores-csv",
    ...
    parser.add_argument("--flip-thresholds", default="0.30,0.40,0.50,0.60,0.70")
...
def choose_threshold(score: np.ndarray, target: np.ndarray, recall_target: float, train_mask: np.ndarray) -> float:
    if int(train_mask.sum()) == 0 or int(target[train_mask].sum()) == 0:
        return 1.01
    threshold, _ = select_threshold(score[train_mask], target[train_mask], recall_target)
...
def route_for_fold(
    ...
    if route_mode == "common_hard":
        threshold = choose_threshold(common_arr, hard_true, recall_target, train)
        test_flag = test & (common_arr >= threshold)
        train_flags = {0: train & (common_arr >= threshold), 1: train & (common_arr >= threshold)}
        info = {"low_threshold": threshold, "high_threshold": threshold}
    elif route_mode == "directional_error":
        low_train = train & (base_pred == 0)
        high_train = train & (base_pred == 1)
        low_threshold = choose_threshold(low_arr, upper_fn, recall_target, low_train)
        high_threshold = choose_threshold(high_arr, upper_fp, recall_target, high_train)
        test_flag = test & (
        ...
        info = {"low_threshold": low_threshold, "high_threshold": high_threshold}
    else:
        raise ValueError(route_mode)
    return test_flag, train_flags, info
...
def evaluate_policy(
    ...
    flip_prob_all = np.full(len(df), np.nan)
    route_flag = np.zeros(len(df), dtype=bool)
    flip_applied = np.zeros(len(df), dtype=bool)
    fold_rows: list[dict[str, Any]] = []

    for fold in sorted(np.unique(folds)):
        train = folds != fold
        test_flag, train_flags, info = route_for_fold(
            df=df,
            fold=int(fold),
        ...
        for direction in [0, 1]:
            ...
            if train_scope == "flag_train":
                dir_train = train_flags[direction] & (base_pred == direction)
            elif train_scope == "all_train":
                dir_train = train & (base_pred == direction)
            else:
                raise ValueError(train_scope)
            y_flip = (y[dir_train] != base_pred[dir_train]).astype(int)
            _, flip_prob = fit_predict(model, x[dir_train], y_flip, x[dir_test])
            flip_prob_all[dir_test] = flip_prob
            flip = flip_prob >= flip_threshold
            flip_applied[dir_test] = flip
            if direction == 0:
                final_pred[dir_test] = flip.astype(int)
                final_prob[dir_test] = flip_prob
            else:
                final_pred[dir_test] = (~flip).astype(int)
                final_prob[dir_test] = 1.0 - flip_prob
        ...
        fold_rows.append(
            {
                ...
                "test_flip_n": int((flip_applied & (folds == fold)).sum()),
    ...
    case_df["flip_threshold"] = float(flip_threshold)
    case_df["route_flag"] = route_flag.astype(int)
    case_df["flip_prob"] = flip_prob_all
    case_df["flip_applied"] = flip_applied.astype(int)
...
def main() -> None:
    ...
    flip_thresholds = [float(item) for item in split_arg(args.flip_thresholds)]
                ...
                for recall_target in recall_targets:
                    for model_name, model in models.items():
                        for train_scope in train_scopes:
                            for flip_threshold in flip_thresholds:
                                summary, case_df, fold_df = evaluate_policy(
                                    df=df,
                                    x_df=x_df,
                                    route_mode=route_mode,
                                    common_score=common_score,
                                ...
                                fold_df["flip_threshold"] = flip_threshold
                                cases.append(case_df)
                                folds.append(fold_df)

    summary_df = pd.DataFrame(summaries).sort_values(["final_bacc", "final_acc", "final_auc"], ascending=False)
    summary_df.to_csv(out_dir / "directional_flip_policy_summary.csv", index=False)
    pd.concat(cases, ignore_index=True).to_csv(out_dir / "directional_flip_policy_case_outputs.csv", index=False)
    pd.concat(folds, ignore_index=True).to_csv(out_dir / "directional_flip_policy_fold_thresholds.csv", index=False)
