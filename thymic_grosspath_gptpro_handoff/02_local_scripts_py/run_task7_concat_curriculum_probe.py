...
TASK6_LOW_RISK_IDXS = {0, 1, 2}
TASK6_HIGH_RISK_IDXS = {3, 4, 5}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run curriculum case-level MLP on dual DINO frozen concat features.")
    ...
    parser.add_argument(
        "--selection-metric",
        default="accuracy",
        choices=("primary", "accuracy", "balanced_accuracy", "f1", "macro_f1", "macro_recall"),
    )
    ...
    parser.add_argument(
        "--easy-exclude-label-idxs",
        default="",
        help="Comma-separated class indices that should never be treated as easy, e.g. '3,4' for B2/B3 in Task6.",
    )
    ...
    parser.add_argument(
        "--write-task7-folded",
        action="store_true",
        help="When task=task6_sixclass, also write a Task7-style binary run by summing A/AB/B1 vs B2/B3/TC.",
    )
    parser.add_argument("--task7-folded-threshold", type=float, default=0.50)
...
def parse_idx_set(text: str) -> set[int]:
    return {int(item.strip()) for item in str(text).split(",") if item.strip()}


def load_curriculum_table(teacher_oof_csvs: list[Path], args: argparse.Namespace, task) -> pd.DataFrame:
    ...
    for idx, path in enumerate(teacher_oof_csvs):
        ...
        need = ["case_id", "label_idx", "pred_idx", "fold_id", *prob_cols]
    ...
    for name in teacher_names:
        if not (merged[f"{name}_label_idx"].astype(int) == merged["label_idx"]).all():
            raise ValueError(f"Teacher {name} label_idx is inconsistent with t1.")
        if not (merged[f"{name}_fold_id"].astype(int) == merged["fold_id"]).all():
            raise ValueError(f"Teacher {name} fold_id is inconsistent with t1.")
        merged[f"{name}_correct"] = (merged[f"{name}_pred_idx"].astype(int) == merged["label_idx"]).astype(int)
        probs = merged[[f"{name}_{col}" for col in prob_cols]].to_numpy(dtype=np.float64)
        labels = merged["label_idx"].to_numpy(dtype=np.int64)
        merged[f"{name}_true_prob"] = probs[np.arange(len(merged)), labels]
        sorted_probs = np.sort(probs, axis=1)
        if probs.shape[1] == 1:
            merged[f"{name}_margin"] = 1.0
        else:
            merged[f"{name}_margin"] = sorted_probs[:, -1] - sorted_probs[:, -2]
    ...
    easy_mask = (
        ...
        & (merged["mean_margin"] >= args.easy_mean_margin)
    )
    easy_excluded = parse_idx_set(args.easy_exclude_label_idxs)
    if easy_excluded:
        easy_mask = easy_mask & ~merged["label_idx"].isin(easy_excluded)
    medium_mask = (
        ~easy_mask
        & (merged["correct_count"] >= args.medium_min_correct)
        & (merged["mean_true_prob"] >= args.medium_mean_true_prob)
        & (merged["mean_margin"] >= args.medium_mean_margin)
    )
    merged["difficulty"] = np.where(easy_mask, "easy", np.where(medium_mask, "medium", "hard"))
    hard_salvage_mask = (
        (merged["difficulty"] == "hard")
        & (merged["correct_count"] >= 1)
        & (merged["mean_true_prob"] >= args.hard_salvage_mean_true_prob)
        & (merged["mean_margin"] >= args.hard_salvage_mean_margin)
    )
...
def fold_task6_predictions_to_task7(
    task6_df: pd.DataFrame,
    threshold: float = 0.50,
) -> pd.DataFrame:
    required = {"case_id", "label_idx", *[f"prob_{name}" for name in TASK6_CLASS_NAMES]}
    missing = required.difference(task6_df.columns)
    if missing:
        raise KeyError(f"Task6 predictions missing columns for Task7 folding: {sorted(missing)}")
    out = task6_df.copy()
    out["prob_low_risk_group"] = out[["prob_A", "prob_AB", "prob_B1"]].sum(axis=1)
    out["prob_high_risk_group"] = out[["prob_B2", "prob_B3", "prob_TC"]].sum(axis=1)
    out["label_idx"] = out["label_idx"].astype(int).map(lambda x: 0 if x in TASK6_LOW_RISK_IDXS else 1)
    out["pred_idx"] = (out["prob_high_risk_group"] >= float(threshold)).astype(int)
...
def write_folded_task7_split(
    output_dir: Path,
    fold_id: int,
    split: str,
    task6_predictions: pd.DataFrame,
    threshold: float,
) -> None:
    folded_dir = output_dir / "task7_folded_from_task6" / f"fold_{fold_id}"
    folded_dir.mkdir(parents=True, exist_ok=True)
    folded = fold_task6_predictions_to_task7(task6_predictions, threshold=threshold)
    folded.to_csv(folded_dir / f"{split}_case_predictions_mean.csv", index=False)


def write_folded_task7_oof(output_dir: Path, threshold: float) -> None:
    task6_oof_path = output_dir / "oof_case_predictions_mean.csv"
    if not task6_oof_path.exists():
        return
    folded_dir = output_dir / "task7_folded_from_task6"
    folded_dir.mkdir(parents=True, exist_ok=True)
    task6_oof = pd.read_csv(task6_oof_path, dtype={"case_id": str})
    folded = fold_task6_predictions_to_task7(task6_oof, threshold=threshold)
    folded.to_csv(folded_dir / "oof_case_predictions_mean.csv", index=False)
    metrics = summarize_prediction_frame(
        folded[["case_id", "label_idx", "pred_idx", "prob_low_risk_group", "prob_high_risk_group"]],
        TASK7_CLASS_NAMES,
    )
    pd.DataFrame([metrics]).to_csv(folded_dir / "oof_metrics.csv", index=False)
    with (folded_dir / "overall_metrics.json").open("w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    difficulty_df = load_curriculum_table(teacher_oof_csvs, args, task)
    ...
    for fold_id in fold_ids:
        ...
        run_config = {
            ...
            "easy_rule": {
                ...
                "mean_margin": float(args.easy_mean_margin),
                "exclude_label_idxs": sorted(parse_idx_set(args.easy_exclude_label_idxs)),
            },
            "medium_rule": {
                "min_correct": int(args.medium_min_correct),
                "mean_true_prob": float(args.medium_mean_true_prob),
                "mean_margin": float(args.medium_mean_margin),
            },
            "hard_salvage_rule": {
                "mean_true_prob": float(args.hard_salvage_mean_true_prob),
                "mean_margin": float(args.hard_salvage_mean_margin),
            },
            "hard_weight": float(args.hard_weight),
            "stage3_hard_mode": args.stage3_hard_mode,
            "write_task7_folded": bool(args.write_task7_folded),
            "task7_folded_threshold": float(args.task7_folded_threshold),
            "fold_id": int(fold_id),
        }
        ...
        if args.write_task7_folded:
            if task.class_names != TASK6_CLASS_NAMES:
                raise ValueError("--write-task7-folded is only valid when task=task6_sixclass.")
            write_folded_task7_split(
                output_dir=output_dir,
                fold_id=fold_id,
                split="val",
                task6_predictions=full_val_predictions,
                threshold=args.task7_folded_threshold,
            )
            write_folded_task7_split(
                output_dir=output_dir,
                fold_id=fold_id,
                split="test",
                task6_predictions=test_predictions,
                threshold=args.task7_folded_threshold,
            )
    ...
    if len(fold_ids) > 1:
        ...
        if args.write_task7_folded:
            write_folded_task7_oof(output_dir, threshold=args.task7_folded_threshold)
