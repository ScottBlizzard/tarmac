...
OUT_DIR = ROOT / "outputs" / "grosspath_rc_v60_specialist_feature_ablation_20260527"
BASE_POLICY = "fusion_rank::dir_plus_pred_low_fn::budget=0.650"
ADDON_RATES = np.round(np.arange(0.0, 0.131, 0.01), 3)
...
NON_DEPLOYABLE_GROSS_COLS = {
    "gross_text",
    "n_gross_rows",
    "concept_has_gross_text",
    "gross_text_len",
    "pleura_attached",
    "pleura_attached_count",
    "all_max_area_mm2",
...
def common(cols: list[str], dev: pd.DataFrame, ext: pd.DataFrame) -> list[str]:
    out = []
    for c in cols:
        if c in dev.columns and c in ext.columns and c not in out:
            out.append(c)
    return out


def feature_sets(dev: pd.DataFrame, ext: pd.DataFrame) -> dict[str, tuple[list[str], list[str]]]:
    prob = common(v30.PROB_FEATURES + v30.BIN_FEATURES, dev, ext)
    img = common(v30.IMAGE_FEATURES, dev, ext)
    cat = common(v30.CAT_FEATURES, dev, ext)
    v59_extra = common(
        [
            "prob103_vitl",
            "prob107_qkvb",
    ...
    all_num = []
    for block in [prob, img, v59_extra]:
        for c in block:
            if c not in all_num:
                all_num.append(c)
    deployable_num = [c for c in all_num if c not in NON_DEPLOYABLE_GROSS_COLS]
    return {
        "model_prob_only": (prob, []),
        "model_prob_plus_view": (prob, cat),
        "model_prob_image": (prob + [c for c in img if c not in prob], cat),
        "deployable_no_gross": (deployable_num, cat),
        "all_with_gross_concepts": (all_num, cat),
    }
...
def specialist_scores(
    ...
    y = train["label_idx"].astype(int).eq(0).astype(int).to_numpy()
    ...
    try:
        ...
        ext_auc = float("nan")
    meta = {
        "feature_set_n_numeric": len(numeric),
        "feature_set_n_categorical": len(categorical),
...
def build_grid() -> pd.DataFrame:
    ...
    _grid54, ext_reviews = v54.build_policy_grid(dev, ext, dev_scores, ext_scores)
    ...
    rows = []
    meta_rows = []
    for fs_name, (numeric, categorical) in feature_sets(dev, ext).items():
        dev_score, ext_score, meta = specialist_scores(dev, ext, numeric, categorical)
        meta_rows.append({"feature_set": fs_name, **meta})
    ...
    return grid.merge(meta, on="feature_set", how="left")
...
def main() -> None:
    ...
    meta_cols = [
        "feature_set",
        "feature_set_n_numeric",
        "feature_set_n_categorical",
        "dev_pred_high_n",
        "dev_false_positive_target_n",
        "external_pred_high_n",
        "external_false_positive_target_n",
        "dev_auc",
        "external_auc",
    ]
    meta = grid[meta_cols].drop_duplicates("feature_set")
    grid.to_csv(OUT_DIR / "v60_specialist_feature_ablation_grid.csv", index=False, encoding="utf-8-sig")
    meta.to_csv(OUT_DIR / "v60_specialist_feature_ablation_auc.csv", index=False, encoding="utf-8-sig")
    selected.to_csv(OUT_DIR / "v60_dev_selected_feature_ablation.csv", index=False, encoding="utf-8-sig")
    ...
    print("Feature-set AUC:")
    print(meta.to_string(index=False))
    print("\nDev-selected feature ablation:")
