...
OUT_DIR = ROOT / "outputs" / "grosspath_rc_v126_leave_domain_two_signal_validation_20260527"
...
def scan_rules(train: pd.DataFrame, base_review: pd.Series) -> pd.DataFrame:
    rows = []
    for wc_max in np.round(np.arange(0.400, 0.901, 0.025), 3):
        for core_max in np.round(np.arange(0.650, 0.901, 0.025), 3):
...
def select_rule(train: pd.DataFrame, base_review: pd.Series) -> pd.Series:
    base_m = metrics(train, base_review)
    scan = scan_rules(train, base_review)
    # Safety constraint: do not increase FN, eliminate FP when possible.
...
def main() -> None:
    ...
    df["leave_domain_extra_review"] = False
    rule_rows = []
    transfer_rows = []
    ...
    for holdout in internal_domains:
        ...
        selected = select_rule(train, train["v111_review_or_control"].astype(bool))
        hold_extra = fp_guard(
            hold,
            hold["v111_review_or_control"].astype(bool),
            float(selected["wc_max"]),
            float(selected["core_max"]),
        )
        df.loc[hold.index, "leave_domain_extra_review"] = hold_extra.to_numpy(bool)
        hold_review = hold["v111_review_or_control"].astype(bool) | hold_extra
        train_extra = fp_guard(
            train,
            train["v111_review_or_control"].astype(bool),
            float(selected["wc_max"]),
            float(selected["core_max"]),
        )
        train_review = train["v111_review_or_control"].astype(bool) | train_extra
        rule_rows.append(
            {
                ...
                "train_remaining_error_n": int(metrics(train, train_review)["remaining_error_n"]),
                "train_fn": int(metrics(train, train_review)["fn"]),
                "train_fp": int(metrics(train, train_review)["fp"]),
                ...
                "holdout_remaining_error_n": int(metrics(hold, hold_review)["remaining_error_n"]),
                "holdout_fn": int(metrics(hold, hold_review)["fn"]),
                "holdout_fp": int(metrics(hold, hold_review)["fp"]),
                "holdout_bacc": float(metrics(hold, hold_review)["balanced_accuracy"]),
            }
        )
        ...
        transfer_rows.append(rescued)

    df["leave_domain_review_or_control"] = df["v111_review_or_control"].astype(bool) | df["leave_domain_extra_review"].astype(bool)
    df["leave_domain_rescued_error"] = df["leave_domain_extra_review"].astype(bool) & df["final_correct"].eq(0)
    df["leave_domain_extra_clean_review"] = df["leave_domain_extra_review"].astype(bool) & df["final_correct"].eq(1)
    ...
    summary = pd.concat(
        [
            ...
            evaluate_scope_rows(df, "leave_domain_review_or_control", "leave_domain_selected_two_signal"),
        ],
        ignore_index=True,
    )
    rules = pd.DataFrame(rule_rows)
    rescued_cases = pd.concat(transfer_rows, ignore_index=True) if transfer_rows else pd.DataFrame()

    summary.to_csv(OUT_DIR / "v126_leave_domain_summary.csv", index=False, encoding="utf-8-sig")
    format_table(summary).to_csv(OUT_DIR / "v126_leave_domain_summary_formatted.csv", index=False, encoding="utf-8-sig")
    rules.to_csv(OUT_DIR / "v126_leave_domain_selected_rules.csv", index=False, encoding="utf-8-sig")
    rescued_cases.to_csv(OUT_DIR / "v126_leave_domain_rescued_cases.csv", index=False, encoding="utf-8-sig")
    df.to_csv(OUT_DIR / "v126_leave_domain_cases.csv", index=False, encoding="utf-8-sig")

    focus = summary[
        summary["workflow"].eq("leave_domain_selected_two_signal") & summary["scope"].eq("all_domains")
    ].iloc[0]
    third = summary[
        summary["workflow"].eq("leave_domain_selected_two_signal") & summary["scope"].eq("third_batch")
    ].iloc[0]
    old = summary[
        summary["workflow"].eq("leave_domain_selected_two_signal") & summary["scope"].eq("old_data")
    ].iloc[0]
    rescued_text = "; ".join(rescued_cases["original_case_id"].astype(str).tolist()) if not rescued_cases.empty else "none"
    lines = [
        "# v126 Leave-domain Two-signal Validation",
        "",
        "v126 selects the two-signal FP guard on one internal domain and applies it to the other internal domain.",
        "",
        f"- Leave-domain all-domain: BAcc {pct(focus['balanced_accuracy'])}, control {pct(focus['control_rate'])}, FN={int(focus['fn'])}, FP={int(focus['fp'])}.",
        f"- Old data: BAcc {pct(old['balanced_accuracy'])}, FN={int(old['fn'])}, FP={int(old['fp'])}.",
        f"- Third batch: BAcc {pct(third['balanced_accuracy'])}, FN={int(third['fn'])}, FP={int(third['fp'])}.",
        f"- Held-out rescued cases: {rescued_text}.",
        "",
        "This tests whether the low-risk overcall guard transfers between old and third internal domains instead of only fitting their pooled data.",
    ]
    (OUT_DIR / "v126_key_messages.md").write_text("\n".join(lines), encoding="utf-8-sig")
