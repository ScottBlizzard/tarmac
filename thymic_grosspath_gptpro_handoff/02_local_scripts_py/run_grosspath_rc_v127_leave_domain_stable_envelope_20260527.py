...
OUT_DIR = ROOT / "outputs" / "grosspath_rc_v127_leave_domain_stable_envelope_20260527"
...
def select_min_rule(scan: pd.DataFrame, base_m: dict[str, float | int]) -> pd.Series:
...
def select_stable_envelope(scan: pd.DataFrame, base_m: dict[str, float | int], cushion: int = 6) -> pd.Series:
    candidates = scan[(scan["fn"].le(base_m["fn"])) & (scan["fp"].eq(0))].copy()
    if candidates.empty:
        candidates = scan.copy()
...
def main() -> None:
    ...
    for selector in ["min_review", "stable_envelope"]:
        df[f"{selector}_extra_review"] = False
    ...
    for holdout in internal_domains:
        ...
        selected_map = {
            "min_review": select_min_rule(scan, base_m),
            "stable_envelope": select_stable_envelope(scan, base_m, cushion=6),
        }
        for selector, selected in selected_map.items():
            ...
            df.loc[hold.index, f"{selector}_extra_review"] = hold_extra.to_numpy(bool)
            ...
            rescued["selector"] = selector
            rescued["train_domain"] = train_domain
            rescued_rows.append(rescued)

    for selector in ["min_review", "stable_envelope"]:
        df[f"{selector}_review_or_control"] = df["v111_review_or_control"].astype(bool) | df[f"{selector}_extra_review"].astype(bool)
        df[f"{selector}_rescued_error"] = df[f"{selector}_extra_review"].astype(bool) & df["final_correct"].eq(0)
        df[f"{selector}_extra_clean_review"] = df[f"{selector}_extra_review"].astype(bool) & df["final_correct"].eq(1)
    ...
    summary = pd.concat(
        [
            ...
            evaluate_scope_rows(df, "min_review_review_or_control", "leave_domain_min_review"),
            evaluate_scope_rows(df, "stable_envelope_review_or_control", "leave_domain_stable_envelope"),
        ],
        ignore_index=True,
    )
    rules = pd.DataFrame(rule_rows)
    rescued_cases = pd.concat(rescued_rows, ignore_index=True) if rescued_rows else pd.DataFrame()
    summary.to_csv(OUT_DIR / "v127_leave_domain_stable_summary.csv", index=False, encoding="utf-8-sig")
    format_table(summary).to_csv(OUT_DIR / "v127_leave_domain_stable_summary_formatted.csv", index=False, encoding="utf-8-sig")
    rules.to_csv(OUT_DIR / "v127_leave_domain_stable_rules.csv", index=False, encoding="utf-8-sig")
    format_table(rules).to_csv(OUT_DIR / "v127_leave_domain_stable_rules_formatted.csv", index=False, encoding="utf-8-sig")
    rescued_cases.to_csv(OUT_DIR / "v127_leave_domain_stable_rescued_cases.csv", index=False, encoding="utf-8-sig")
    df.to_csv(OUT_DIR / "v127_leave_domain_stable_cases.csv", index=False, encoding="utf-8-sig")

    stable_all = summary[
        summary["workflow"].eq("leave_domain_stable_envelope") & summary["scope"].eq("all_domains")
    ].iloc[0]
    min_all = summary[
        summary["workflow"].eq("leave_domain_min_review") & summary["scope"].eq("all_domains")
    ].iloc[0]
    stable_rescued = rescued_cases[rescued_cases["selector"].eq("stable_envelope")] if not rescued_cases.empty else pd.DataFrame()
    rescued_text = "; ".join(stable_rescued["original_case_id"].astype(str).tolist()) if not stable_rescued.empty else "none"
    lines = [
        "# v127 Leave-domain Stable-envelope Validation",
        "",
        "v127 compares minimum-review leave-domain rule selection with a broader stable-envelope rule selection.",
        "",
        f"- Minimum-review leave-domain: BAcc {pct(min_all['balanced_accuracy'])}, control {pct(min_all['control_rate'])}, FN={int(min_all['fn'])}, FP={int(min_all['fp'])}.",
        f"- Stable-envelope leave-domain: BAcc {pct(stable_all['balanced_accuracy'])}, control {pct(stable_all['control_rate'])}, FN={int(stable_all['fn'])}, FP={int(stable_all['fp'])}.",
        f"- Stable-envelope held-out rescued cases: {rescued_text}.",
        "",
        "This tests whether the v115/v117 stable-envelope principle improves cross-internal-domain transfer.",
    ]
    (OUT_DIR / "v127_key_messages.md").write_text("\n".join(lines), encoding="utf-8-sig")
