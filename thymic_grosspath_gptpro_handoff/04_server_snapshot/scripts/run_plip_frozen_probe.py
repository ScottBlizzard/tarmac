...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a frozen PLIP feature probe on thymic tasks.")
    ...
    parser.add_argument("--model-dir", default="third_party/round3/local_models/plip", help="Local path to PLIP model files.")
...
def extract_image_features(model: CLIPModel, batch_inputs: Any) -> torch.Tensor:
    if isinstance(batch_inputs, torch.Tensor):
        return model.get_image_features(pixel_values=batch_inputs)
...
def run_single_fold(
    ...
    print(
        f"\n=== PLIP Fold {fold_id}/{fold_count} | task={task.key} | variant={args.input_variant} ===",
        flush=True,
    )
    print(
...
def main() -> None:
    ...
    model, processor = load_plip_model(model_dir=model_dir, device=device)

    print(
        f"Starting PLIP frozen probe: task={task.key}, variant={args.input_variant}, "
        f"model_dir={model_dir}, device={device}, folds={fold_ids}",
        flush=True,
    )
    for fold_id in fold_ids:
        run_single_fold(
