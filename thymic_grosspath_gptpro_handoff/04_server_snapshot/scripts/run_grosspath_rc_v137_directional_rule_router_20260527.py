...
BASE_CANDIDATES = ["main_prob", "robust_prob", "prob_mean_core", "stack_gb_d2_core", "stack_gb_d2_multiview"]
FN_SIGNALS = [
    "prob_mean_core",
    "wholecrop_prob",
...
def threshold_grid(values: np.ndarray, mode: str) -> np.ndarray:
    if mode == "high":
        qs = np.linspace(0.45, 0.95, 11)
    else:
        qs = np.linspace(0.05, 0.55, 11)
...
def main() -> None:
    ...
    for base_name in [c for c in BASE_CANDIDATES if c in usable]:
        ...
        for base_obj in ["balanced_accuracy", "high_sensitivity"]:
