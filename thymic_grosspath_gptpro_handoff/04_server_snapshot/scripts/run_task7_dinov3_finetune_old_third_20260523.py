...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune DINOv3/timm on old+third Task7 development data.")
...
def configure_trainable_parameters(model: DINOv3FineTuneModel, tune_scope: str) -> dict[str, int]:
...
def build_optimizer(model: DINOv3FineTuneModel, head_lr: float, backbone_lr: float, weight_decay: float) -> torch.optim.Optimizer:
...
def train_one_epoch_amp(
    ...
    for batch_idx, (inputs, labels, _, _) in enumerate(progress, start=1):
...
def run_single_fold(args: argparse.Namespace, task: TaskConfig, fold_id: int, output_dir: Path) -> dict[str, Any]:
    ...
    print(
        f"\n=== DINOv3 FT Fold {fold_id}/{len(fold_ids)} | task={task.key} | variant={args.input_variant} | "
    ...
    val_results = evaluate_model(model, val_loader, task, device, aggregate_methods, criterion, args.max_eval_batches, f"dinov3 fold{fold_id} val best")
    test_results = evaluate_model(model, test_loader, task, device, aggregate_methods, criterion, args.max_eval_batches, f"dinov3 fold{fold_id} test best")
