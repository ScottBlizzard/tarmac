...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Task7 candidate stacking with fold-wise PCA DINO embeddings.")
    parser.add_argument(
        "--case-scores-csv",
...
def build_raw_top_features(df: pd.DataFrame) -> pd.DataFrame:
    pred_cols = [
        "pred_threeway",
        "pred_main",
        "pred_anti34",
    ...
    for pred_col in pred_cols:
        ...
        if prob_col not in df.columns:
            continue
        out[f"raw_{suffix}_pred"] = pd.to_numeric(df[pred_col], errors="coerce").fillna(0.0).astype(float)
        out[f"raw_{suffix}_prob"] = pd.to_numeric(df[prob_col], errors="coerce").fillna(0.5).astype(float)
    return out.replace([np.inf, -np.inf], np.nan).fillna(0.0)


def load_embeddings(df: pd.DataFrame, table_path: Path, npy_path: Path) -> np.ndarray:
    table = pd.read_csv(table_path, dtype={"case_id": str, "original_case_id": str})
    features = np.load(npy_path)
    if len(table) != features.shape[0]:
        raise ValueError(f"Embedding table/features row mismatch: {len(table)} vs {features.shape[0]}")
    index = {case_id: i for i, case_id in enumerate(table["case_id"].astype(str))}
    missing = [case_id for case_id in df["case_id"].astype(str) if case_id not in index]
    if missing:
        raise KeyError(f"Missing embeddings for {missing[:10]}")
    order = [index[case_id] for case_id in df["case_id"].astype(str)]
...
def evaluate(
    ...
    tabular = None if tabular_df is None else tabular_df.to_numpy(dtype=float)
    pred = np.zeros(len(df), dtype=int)
    prob = np.zeros(len(df), dtype=float)
    for fold in sorted(np.unique(folds)):
        train = folds != fold
        test = folds == fold
        emb_train, emb_test = transform_features(embeddings[train], embeddings[test], pca_components, seed)
        if tabular is None:
            x_train, x_test = emb_train, emb_test
        else:
            x_train = np.concatenate([tabular[train], emb_train], axis=1)
            x_test = np.concatenate([tabular[test], emb_test], axis=1)
        pred[test], prob[test] = fit_predict(model, x_train, y[train], x_test)
    summary = metric_row(y, pred, prob)
    summary["model"] = model_name
    summary["pca_components"] = int(pca_components)
    ...
    case_df["pca_components"] = int(pca_components)
...
def main() -> None:
    ...
    raw_top = build_raw_top_features(df)
    embeddings = load_embeddings(df, Path(args.embedding_table), Path(args.embedding_npy))
    feature_sets: dict[str, pd.DataFrame | None] = {
        "embedding_only": None,
        "corrected_plus_embedding": corrected,
        "rawtop_corrected_plus_embedding": pd.concat([raw_top, corrected], axis=1),
    }
    models = make_models(args.seed)
    summaries: list[dict[str, Any]] = []
    cases: list[pd.DataFrame] = []
    for feature_name, tabular_df in feature_sets.items():
        for pca_components in [4, 8, 16, 32, 64]:
            for model_name, model in models.items():
                summary, case_df = evaluate(df, tabular_df, embeddings, pca_components, model_name, model, args.seed)
    ...
    summary_df.to_csv(out_dir / "embedding_candidate_stacking_summary.csv", index=False)
    pd.concat(cases, ignore_index=True).to_csv(out_dir / "embedding_candidate_stacking_case_outputs.csv", index=False)
