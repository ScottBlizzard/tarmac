...
OUT_ROOT="experiments/base_model_expansion_20260706/outputs/domain_consistency_runs"
EXT_ROOT="experiments/base_model_expansion_20260706/outputs/domain_consistency_external_sweep"
...
train_if_needed() {
  ...
  echo "[train] ${tag}"
  python scripts/run_task7_dinov3_domain_consistency_20260706.py \
    --output-dir "${out_dir}" \
    --fold all \
...
train_if_needed "204_qkvb_style_domainrobust_consistency_cw_20260706" \
  --view-a-profile style_light \
  --view-b-profile domain_robust \
  --class-weighting
eval_external_if_needed "204_qkvb_style_domainrobust_consistency_cw_20260706"

train_if_needed "205_qkvb_style_externalmimic_consistency_nocw_20260706" \
  --view-a-profile style_light \
  --view-b-profile external_mimic \
  --no-class-weighting
eval_external_if_needed "205_qkvb_style_externalmimic_consistency_nocw_20260706"

python - <<'PY'
from pathlib import Path
import pandas as pd

root = Path("experiments/base_model_expansion_20260706/outputs/domain_consistency_external_sweep")
rows = []
...
if rows:
    ...
    out.to_csv(root / "domain_consistency_external_sweep_summary.csv", index=False, encoding="utf-8-sig")
