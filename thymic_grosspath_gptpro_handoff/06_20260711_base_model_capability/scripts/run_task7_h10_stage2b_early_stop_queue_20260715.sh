...
RUN=${CODE}/artifacts/nested_phenotype_early_stop_seed20260713
OUT=${ROOT}/experiments/h10_internal_phenotype_difficulty_redesign_20260715/stage2b_early_stop_before_hard_replay
LOGS=${CODE}/logs_stage2b
...
QUEUE=${CODE}/run_task7_h10_stage2b_early_stop_queue_20260715.sh
...
sha256sum "${TRAINER}" "${ANALYZER}" "${QUEUE}" > "${LOGS}/code.sha256"
...
"${PY}" "${TRAINER}" \
  ...
  --device cuda \
  --max-epochs 16 \
  2>&1 | tee "${LOGS}/early_stop_training.log"
...
"${PY}" "${ANALYZER}" \
  ...
  --output-dir "${OUT}" \
  2>&1 | tee "${LOGS}/early_stop_analysis.log"
