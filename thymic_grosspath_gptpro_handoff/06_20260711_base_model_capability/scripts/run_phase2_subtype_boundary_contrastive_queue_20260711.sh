...
output_root="${experiment_root}/phase2_subtype_boundary_contrastive_lodo"
log_root="/root/thymic_feature_banks_20260711/phase2_subtype_boundary_contrastive_logs"
upstream_status="${experiment_root}/phase2_structured_pooling_screen/PHASE2_STRUCTURED_POOLING.status"
status_file="${output_root}/PHASE2_SUBTYPE_BOUNDARY_CONTRASTIVE.status"
...
trap on_exit EXIT
write_status waiting_for_structured_pooling
...
while true; do
  ...
  case "${upstream}" in
    complete*) break ;;
    failed*)
      printf 'Upstream structured pooling failed: %s\n' "${upstream}" >&2
      exit 3
      ;;
    *) sleep 30 ;;
  esac
...
common=(
  ...
  --no-class-weighting
  --supervised-contrastive-weight 0.03
  --supervised-contrastive-temperature 0.10
  --amp
  --amp-dtype bfloat16
)
...
"${python_bin}" "${runner}" "${common[@]}" \
  ...
  --patience 1 \
  --num-workers 0 \
  --supervised-contrastive-mode cross_source_subtype \
  --max-train-batches 1 \
  --max-eval-batches 1

run_experiment \
  343_aimv2l_lora_subtype_supcon003_source_lodo_20260711 \
  --supervised-contrastive-mode subtype

run_experiment \
  344_aimv2l_lora_crosssource_subtype_supcon003_lodo_20260711 \
  --supervised-contrastive-mode cross_source_subtype

run_experiment \
  345_aimv2l_lora_binary_b1b2_hardneg003_source_lodo_20260711 \
  --supervised-contrastive-mode binary_b1b2 --boundary-negative-weight 3

run_experiment \
  346_aimv2l_lora_crosssource_b1b2_hardneg003_lodo_20260711 \
  --supervised-contrastive-mode cross_source_b1b2 --boundary-negative-weight 3
