...
output_root="/workspace/thymic_project/experiments/base_model_capability_20260711/phase2_siglipl_lora_source_lodo"
log_root="/root/thymic_feature_banks_20260711/phase2_siglipl_lora_logs"
upstream_status="/workspace/thymic_project/experiments/base_model_capability_20260711/phase2_pretrained_pool_screen/PHASE2_PRETRAINED_POOL_SCREEN.status"
status_file="$output_root/PHASE2_SIGLIPL_LORA.status"
...
trap on_exit EXIT
write_status waiting_for_pretrained_pool_screen
...
while true; do
  ...
  case "$upstream" in
    complete*) break ;;
    failed*)
      printf 'Upstream pretrained-pool screen failed: %s\n' "$upstream" >&2
      exit 3
      ;;
    *) sleep 30 ;;
  esac
...
common=(
  ...
  --fold all
  --model-name vit_large_patch16_siglip_384.v2_webli
...
run_experiment \
  305_siglipl_lora_last1_conservative_source_lodo_20260711 \
  --epochs 12 --lora-rank 4 --lora-alpha 8 --lora-last-blocks 1 --lora-lr 5e-6 \
  --no-train-final-norm --train-sampler none --class-weighting

run_experiment \
  306_siglipl_lora_last2_baseline_source_lodo_20260711 \
  --epochs 12 --lora-rank 8 --lora-alpha 16 --lora-last-blocks 2 --lora-lr 1e-5 \
  --train-final-norm --train-sampler none --class-weighting

run_experiment \
  307_siglipl_lora_last2_sourcebalanced_lodo_20260711 \
  --epochs 12 --lora-rank 8 --lora-alpha 16 --lora-last-blocks 2 --lora-lr 1e-5 \
  --train-final-norm --train-sampler source_label_balanced --no-class-weighting

run_experiment \
  308_siglipl_lora_last4_low_lr_sourcebalanced_lodo_20260711 \
  --epochs 16 --patience 5 --lora-rank 8 --lora-alpha 16 --lora-last-blocks 4 --lora-lr 5e-6 \
  --train-final-norm --train-sampler source_label_balanced --no-class-weighting
